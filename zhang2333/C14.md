...
## E
### Problem description
> 给一颗有边权的树（节点数n<=1e5，边权为一位正整数）。。</bn>
  规定一条路径的长为沿途所有边权组成的l位十进制数。。</bn>
  求对于一个给定的正整数m，有多少个有序点对(u,v)(u!=v),使得u到v的路径长度是m的倍数。。</bn>

### Solution
  树的点分治。。稍微加一点容斥思想。。</bn>
  我们注意到所有满足题意的点对(u,v)，令u到lca的距离为u.key，lca到v的距离为v.key，v的深度为v.dep。。</bn>
  则：u.key*\pow(10,v.dep)\+v.key\三0(mod m)</bn>
  \=>u.key三(-v.key)*pow(10,-v.dep);(逆元有扩欧搞一搞。。)</bn>
  然后就很简单了。。</bn>
  \（还有注意u==v的时候）。。</bn>
